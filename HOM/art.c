/*
 * This file taken from libart by Armon Dadgar, under BSD-3 license (see
 * LICENSE file), See: https://github.com/armon/libart
 *
 * Mar 26, 2020, Allan Haldane:
 * Modified to add the art_search_or_insert function.
 */
#include <stdlib.h>
#include <string.h>
#include <strings.h>
...
static void* recursive_insert(art_node *n, art_node **ref, 
                              ...
                              void *value, int depth, int *overwrite) {
    ...
    if (IS_LEAF(n)) {
        ...
        if (!leaf_matches(l, key, key_len, depth)) {
            // Toggle overwrite flag to signal leaf was found
            if (*overwrite == 0) {
                *overwrite = 1;
                return l->value;
            }
...
RECURSE_SEARCH:;
    ...
    if (child) {
        return recursive_insert(*child, child, key, key_len, value, depth+1, overwrite);
    }
...
void* art_insert(art_tree *t, const unsigned char *key, int key_len, void *value) {
    int overwrite = 1;
    void *old = recursive_insert(t->root, &t->root, key, key_len, value, 0, &overwrite);
    if (overwrite == 0) t->size++;
    return old;
}

/**
 * Inserts a new value into the ART tree if not already present
 * @arg t The tree
 * @arg key The key
 * @arg key_len The length of the key
 * @arg value Opaque value.
 * @return NULL if the item was newly inserted, otherwise
 * the pre-existing value pointer is returned.
 */
void* art_search_or_insert(art_tree *t, const unsigned char *key, int key_len, void *value) {
    int overwrite = 0;
    void *old = recursive_insert(t->root, &t->root, key, key_len, value, 0, &overwrite);
    if (overwrite == 1) t->size++;
    return old;
}
